...
data dcontrol_candidates; set ddays; if F=0 and e=1 and day>=1 and day<=200;
data dcontrol_candidates; set dcontrol_candidates; by Pt_ID;
if last.Pt_ID then do; day_last=day; output; end;
data dcontrol_candidates(keep=ite Pt_id day_first day_last control); set dcontrol_candidates; control=0;

data _null_; set dcase nobs=obs end=EOF;
if EOF then call symput“"n_ob”", obs);
run;
...
*the value 1 is given to‘'contro’' when selected as a control; 
...
%end;
data dcontrol2(keep=ite Pt_ID day_case case); set dcontrol;
proc sort data=dcontrol2; by Pt_ID;
data dcontrol2; merge dcontrol2 (in=ina) ddays(in=inb); by Pt_ID; if ina=1 and inb=1;
*dcontrol2 has 61 records for each control;
data dcontrol2; set dcontrol2; if day>=day_case-60 and day<=day_case;
%if &f0v=0 and &f1v=0 %then %do;
data dcontrol2(keep=ite Pt_id case dayCXO ex day day_case); %end;
%else  %do;
data dcontrol2(keep=ite Pt_id case dayCXO z ex day day_case); %end;
set dcontrol2; dayCXO=day_case-day;
%do ii=0 %to 60;
data dcontrol3; set dcontrol2; if dayCXO=&ii;
%if &f0v=0 and &f1v=0 %then %do;
data dcontrol3(rename=(ex=c&ii)); %end;
%else %do;
data dcontrol3(rename=(ex=c&ii z=z&ii)); %end;
set dcontrol3;
*dcontrol4 has one record for one control with status of exposure (and confounder) for 61 days;
%if &ii=0 %then %do; data dcontrol4; set dcontrol3; %end;
%else %do; data dcontrol4; merge dcontrol4(in=ina) dcontrol3(in=inb); by Pt_ID; if ina=1 and inb=1;%end;
%end;
data dpattern_ite; set dcase4 dcontrol4;
data dpattern_ite(drop=dayCXO day); set dpattern_ite;
...
%if &subject=“"Ncas”" or &subject=“"Ycas”" %then %do;
*in dataset, case=1 for cases and case=0 for controls;
data dc(drop=case); set dataset; if case=1 then output; %end;
%if &subject =“"Ncontro”" or &subject =“"Ycontro”" %then %do;
data dc(drop=case); set dataset; if case=0 then output; %end;
%if &subject =“"Nct”" or &subject =“"Yct”" %then %do;
...
proc phreg data=dperiods utsett=OR_SCL0 covout noprint; by ite;
%if &subject=“"Nct”" %then %do;
...
strata Pt_ID;
data OR_SCL0; set OR_SCL0; if _type_”"PARM”";
data OR_SCL0; set OR_SCL0; length variable $ 8;
%if &subject=“"Nct”" %then %do;
variable”"e”"; LOR=ex; output; 
variable”"ex_t”"; LOR=ex_tc; output;%end;
%if &subject =“"Ncas”" or &subject =“"Ncontro”"  %then %do;
variable”"e”"; LOR=ex; %end;
%if &subject=“"Yct”" %then %do; 
variable”"e”"; LOR=ex;  output; 
variable”"ex_t”"; LOR=ex_tc; output; 
variable”"”"; LOR=z; output; 
variable”"z_t”"; LOR=z_tc; output; %end;
%if &subject =“"Ycas”" or &subject =“"Ycontro”" %then %do;
variable”"e”"; LOR=ex; output; 
variable”"”"; LOR=z; output;%end;
...
%if &subject =“"Nct”" or &subject =“"Ncas”" or &subject =“"Ncontro”" %then %do;
case=1; ex=IFN(c0=1,1,0);output;
%do j=1 %to &M;
%let jj=%eval(60/&M*&j);
case=0; ex=IFN(c&jj=1,1,0); output;%end;
%end;
%if &subject =“"Yct”" or &subject =“"Ycas”" or &subject =“"Ycontro”" %then %do;
...
%if &subject =“"Ncas”" or &subject =“"Ycas”" or &subject =“"Ncontro”" or &subject =“"Ycontro”" %then %do; 
...
M=&M; variable=“"e”"; OR_MH=exp(LORM); OR_MH_L=exp(LORM-1.96*LORstd); OR_MH_U=exp(LORM+1.96*LORstd);
...
*OR_G;
data dperiods;set dperiods; time=2-case;
proc phreg data=dperiods utsett=OR_G0 covout noprint; by ite;
%if &subject=“"Nct”" %then %do;
...
strata Pt_ID;
data OR_G0; set OR_G0; if _type_”"PARM”";
...
%if &subject=“"Ncas”" or &subject=“"Ycas”" %then %do; data CXO_Case&DatasetN; set CXO_merge; %end;
%if &subject=“"Ncontro”" or &subject=“"Ycontro”" %then %do; data CXO_tc&DatasetN; set CXO_merge; %end;
%if &subject=“"Nct”" or &subject=“"Yct”" %then %do; data CXO_ctc&DatasetN; set CXO_merge; %end;
%if &subject=“"Ncas”" or &subject=“"Ycas”" %then %do; data CXO_Case&DatasetN; set CXO_merge; %end;
%if &subject=“"Ncontro”" or &subject=“"Ycontro”" %then %do; data CXO_tc&DatasetN; set CXO_merge; %end;
%if &subject=“"Nct”" or &subject=“"Yct”" %then %do; data CXO_ctc&DatasetN; set CXO_merge; %end;
%mend CXO;
%CXO“"Ncas”", 01);%CXO“"Ncontro”", 01);%CXO“"Nct”", 01);
%CXO“"Ncas”", 02);%CXO“"Ncontro”", 02);%CXO“"Nct”", 02);
%CXO“"Ncas”", 03);%CXO“"Ncontro”", 03);%CXO“"Nct”", 03);
%CXO“"Ncas”", 04);%CXO“"Ncontro”", 04);%CXO“"Nct”", 04);
%CXO“"Ycas”", 05);%CXO“"Ycontro”", 05);%CXO“"Yct”", 05); 
%CXO“"Ycas”", 06);%CXO“"Ycontro”", 06);%CXO“"Yct”", 06);
%CXO“"Ycas”", 07);%CXO“"Ycontro”", 07);%CXO“"Yct”", 07);
%CXO“"Ycas”", 08);%CXO“"Ycontro”", 08);%CXO“"Yct”", 08);
run;